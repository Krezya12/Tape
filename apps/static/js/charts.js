...
(function () {
  ...
  function gridColor()  { return isDark() ? COLORS.gridDark  : COLORS.grid; }
  function textColor()  { return isDark() ? '#94A3B8' : '#64748B'; }
  function cardBg()     { return isDark() ? '#131E30' : '#FFFFFF'; }
  ...
  /* ─── Grades Bar Chart ───────────────────────────── */
  window.renderGradesChart = function (canvasId) {
    const canvas = document.getElementById(canvasId);
    if (!canvas || !window.Chart) return;

    const labels = ['Module 1', 'Module 2', 'Module 3', 'Module 4', 'Module 5', 'Module 6'];
    const hw   = [18, 16, 19, 15, 17, 20];
    const cw   = [17, 15, 18, 16, 14, 19];
      ...
      options: {
        ...
        plugins: {
          ...
          legend: {
            ...
            position: 'top',
            align: 'end',
            labels: { color: textColor(), font: baseFont, boxWidth: 10, boxHeight: 10, borderRadius: 3, useBorderRadius: true, padding: 16 },
          },
          tooltip: { ...baseDefaults.plugins.tooltip, mode: 'index', intersect: false },
        },
        scales: {
          ...baseDefaults.scales,
          y: { ...baseDefaults.scales.y, min: 0, max: 20, ticks: { ...baseDefaults.scales.y.ticks, stepSize: 5 } },
        },
      },
    });
  };
    ...
    return new Chart(canvas, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { ...baseDefaults.plugins.tooltip, callbacks: { label: ctx => `${ctx.dataset.label}: ${ctx.raw}%` } },
        },
        scales: {
          ...baseDefaults.scales,
          y: { ...baseDefaults.scales.y, min: 50, max: 100, ticks: { ...baseDefaults.scales.y.ticks, callback: v => v + '%' } },
        },
      },
    });
  };
  ...
  window.renderAdminOverviewChart = function (canvasId) {
    ...
    return new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          ...baseDefaults.scales,
          y: { ...baseDefaults.scales.y, min: 60, max: 100, ticks: { ...baseDefaults.scales.y.ticks, callback: v => v + '%' } },
        },
      },
    });
  };
      ...
      options: {
        ...
        plugins: {
          legend: {
            display: true,
            position: 'bottom',
            labels: { color: textColor(), font: baseFont, padding: 16, boxWidth: 10, boxHeight: 10, borderRadius: 3, useBorderRadius: true },
          },
